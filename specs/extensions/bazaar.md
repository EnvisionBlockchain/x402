...
extensions.bazaar.evidenceShape:
  ...
  observation_window:
    type: rolling
    duration: P30D    # ISO 8601, 30-day rolling window
    declared_at: "2026-05-15"
```
...
The `observation_window` declaration (per the privacy_class spec, #2334) is part of the count's identity for cross-observer comparison: counts from different observers with different windows may legitimately differ, and the window MUST be readable from the registry entry.
